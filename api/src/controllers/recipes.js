...
const getRecipes = async(req, res, next) => {
    try{
        ...
        //#region name
        if(name && name !== ""){ //
            const allRecipesApi = await axios.get(`https://api.spoonacular.com/recipes/complexSearch?apiKey=6da9637be7e44bfaa91d3e73be34d4a6&addRecipeInformation=true&query=${name}&number=100`)
            ...
            const allRecipesApi = await axios.get("https://api.spoonacular.com/recipes/complexSearch?apiKey=6da9637be7e44bfaa91d3e73be34d4a6&addRecipeInformation=true&number=100")
            myInfo = await allRecipesApi.data.results.map(el => {
                return {
                    image: el.image,
                    name: el.title,
            ...
            const recipesDb = allRecipesDb.map(el =>{
                const diets = el.diets.map(el => el.name)
                return {
                    image: null,
                    name: el.name,
                    diets,
            ...
            allData = recipesDb.concat(myInfo)
        }
        //#endregion
...
const getRecipesById = async(req, res, next) => {
    ...
    if(id){
        try {
            if(!id.includes("-")){
                const idApi = await axios.get(`https://api.spoonacular.com/recipes/${id}/information?apiKey=6da9637be7e44bfaa91d3e73be34d4a6`)
                    const info = {
...
const postRecipes = async(req, res, next) => {
    try{
        ...
        diets.forEach(async el =>{
            const diet = await Diet.findOrCreate({where:{name: el}})
            await createRecipe.setDiets(diet[0].id)
        })
